...
public:
	...
	Impl(int log_level, std::string file);
	~Impl() {};
...
Logging::Impl::Impl(int log_level, std::string file)
{
	...
	if (std::filesystem::exists(file_path_))
	{
		...
		if (size > MAX_LOG_SIZE * 1000)
		{
			...
			if (is)
			{
				is.seekg(-(MAX_LOG_SIZE_TRIM * 1000), std::ios::end);
...
Logging::Logging(int log_level, std::string log_file)
